...
if contract_address and os.path.exists(abi_file_path):
    try:
        ...
        with col1:
            ...
            tab1, tab2 = st.tabs(["🌱 Harvest (Start)", "🚚 Update Status (Move)"])
            ...
            # ACTION B: MOVE / UPDATE
            with tab2:
                st.write("Change the status of an existing batch.")
                update_id = st.number_input("Batch ID to Update", min_value=1, step=1)
                
                # Mapped to Enum: 0=Harvested, 1=Processed, 2=Packed, 3=Shipped, 4=Received, 5=Sold
                status_options = {
                    "Processed": 1, 
                    "Packed": 2, 
                    "Shipped": 3, 
                ...
                new_status = st.selectbox("New Status", list(status_options.keys()))
                ...
                if st.button("Update Status"):
                    try:
                        ...
                        st.error(f"Error: {e}")
                        st.warning("Note: Only the current owner can update the status!")
        ...
        with col2:
            ...
            if st.button("Trace Product"):
                try:
                    ...
                    st.error("Error reading chain.")

    except Exception as e:
        st.error("JSON Error. Please check ChainProof.json")
else:
